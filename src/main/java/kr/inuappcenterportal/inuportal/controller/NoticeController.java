...
public class NoticeController {
    ...
    @Operation(summary = "모든 공지사항 가져오기",description = "url 파라미터에 정렬기준(sort) 을 빈값(최신순), view 둘 중 하나로 보내주세요")
    @ApiResponses({
    ...
    @GetMapping("")
    public ResponseEntity<ResponseDto<List<NoticeListResponseDto>>> getAllNotice(@RequestParam(required = false) String sort){
    ...
    @Operation(summary = "카테고리별 공지사항 가져오기",description = "url 파라미터에 공지사항 카테고리,정렬기준(sort) 을 빈값(최신순),date 둘 중 하나로 보내주세요")
    @ApiResponses({
    ...
    @GetMapping("/{category}")
    public ResponseEntity<ResponseDto<List<NoticeListResponseDto>>> getNoticeByCategory(@PathVariable String category, @RequestParam(required = false) String sort){
        log.info("카테고리별 공지사항 가져오기 카테고리:{}",category);
        return new ResponseEntity<>(new ResponseDto<>(noticeService.getNoticeByCategory(category,sort),"카테고리별 공지사항 가져오기 성공"),HttpStatus.OK);
    }
}
