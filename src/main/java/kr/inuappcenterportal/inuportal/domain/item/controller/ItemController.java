...
public class ItemController {
    ...
    @Operation(summary = "물품 저장", description = "헤더 Auth에 발급받은 토큰을, 바디에 {itemCategory,name,int 형태의 totalQuantity, deposit} 보내주세요. ")
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "물품 등록 성공", content = @Content(schema = @Schema(implementation = ResponseDto.class)))
    })
    @PostMapping
    public ResponseEntity<ResponseDto<Long>> register(@RequestBody @Valid ItemRegister itemRegister) {
        return ResponseEntity.status(HttpStatus.CREATED).body(ResponseDto.of(itemService.register(Item.create(itemRegister)), "물품 등록 성공"));
    }
    ...
    @Operation(summary = "물품 수정", description = "헤더 Auth에 발급받은 토큰을 보내주세요. 바디에 {itemCategory,name,int 형태의 totalQuantity, deposit} , url 파라미터에 물품의 id를 보내주세요.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "책 리스트 조회 성공", content = @Content(schema = @Schema(implementation = ListResponseDto.class)))
    })
    @PutMapping("/{itemId}")
    public ResponseEntity<ResponseDto<Long>> update(@RequestBody @Valid ItemUpdate itemUpdate, @PathVariable Long itemId) {
        itemService.update(itemUpdate, itemId);
    ...
    @Operation(summary = "물품 리스트 조회", description = "헤더 Auth에 발급받은 토큰을 보내주세요. url 파라미터에 물품의 id를 보내주세요.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "책 리스트 조회 성공", content = @Content(schema = @Schema(implementation = ResponseDto.class)))
    })
    @GetMapping
