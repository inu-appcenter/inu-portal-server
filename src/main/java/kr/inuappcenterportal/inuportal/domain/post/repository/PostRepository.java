...
public interface PostRepository extends JpaRepository<Post,Long> {
    ...
    @Query(" SELECT p FROM Post p WHERE p.isDeleted = false " +
            ...
            "AND (:blockedMemberIds IS NULL OR p.member.id NOT IN :blockedMemberIds)")
    Page<Post> findAllByCategoryExcludingPostIds(@Param("category") String category, @Param("postIds") List<Long> postIds, @Param("blockedMemberIds") List<Long> blockedMemberIds, Pageable pageable);
    ...
    Page<Post> findAllRandomized(@Param("category") String category, @Param("postIds") List<Long> postIds, @Param("blockedMemberIds") List<Long> blockedMemberIds, Pageable pageable);
    ...
    @Query(" SELECT p FROM Post p WHERE p.isDeleted = false " +
            ...
            "AND (:postIds IS NULL OR p.id NOT IN :postIds) " +
            "AND (:blockedMemberIds IS NULL OR p.member.id NOT IN :blockedMemberIds)")
    List<Post> findFilteredPosts(@Param("category") String category, @Param("id") Long id, @Param("postIds") List<Long> postIds, @Param("blockedMemberIds") List<Long> blockedMemberIds, Pageable pageable);
}
