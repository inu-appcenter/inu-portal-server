...
%s
                ...
                - 도서관 열람실 좌석 또는 스터디룸 예약/배정 요청(예: '제1노트북실 25번 좌석 예약해줘', '205호 스터디룸 예약해줘', '열람실 예약')은 'LIBRARY' 도구를 호출하세요. 사용자가 언급한 열람실명(roomName), 좌석 번호(seatNo), 스터디룸명, 시간 등을 파라미터로 충실히 전달하세요.
    ...
    private SynthesizedResult synthesizeAnswer(String userMessage, List<ChatMessageDto> history, String toolSummary) {
        ...
        String styleGuideline = containsInuAi ? """
                ...
                2. [학과 사무실(과사) 번호 구분]: 사용자가 '과사(학과 사무실)', '사무실 전화번호', '사무실 위치'를 물어본 경우, 시스템 데이터에 명시된 [학과 사무실(과사)]의 번호와 위치를 안내해야 하며, 교수님 개인 전화번호나 연구실 번호를 학과 사무실 번호로 오인하여 안내하지 마세요.
                3. [도서관 좌석 및 스터디룸 예약 원칙]: 도서관 열람실 좌석 배정 및 스터디룸 예약은 앱 내 보안 인증을 거쳐야 하므로 화면의 인터랙티브 카드에서 사용자가 [확인 및 배정/예약 신청하기] 버튼을 직접 눌러야 시스템에 반영됩니다. 텍스트로 '배정(예약)이 완료되었습니다'라고 거짓말하지 말고, '아래 카드에서 좌석/시간 정보를 확인하시고 [배정 신청/예약하기] 버튼을 눌러주세요'라고 안내하세요.
                4. 시간표, 학식, 버스, 일정 등 캠퍼스 생활 정보는 핵심 위주로 명확하고 깔끔하게 요약하세요.
                5. [엄격한 사실 근거]: 반드시 주어진 시스템 데이터의 실제 내용만을 바탕으로 안내하며, 임의의 사실이나 시스템 메뉴를 지어내지 마세요.
                """;
