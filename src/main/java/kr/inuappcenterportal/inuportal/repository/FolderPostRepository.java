...
public interface FolderPostRepository extends JpaRepository<FolderPost,Long> {
    ...
    @Query("SELECT f FROM FolderPost f JOIN FETCH f.post p WHERE f.folder=:folder and (f.post.content LIKE concat('%',:content,'%') or f.post.title LIKE concat('%',:content,'%')) ORDER BY p.id desc ")
